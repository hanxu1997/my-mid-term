function [A] = getLaplacian1(I, epsilon, win_size)
  ...
  %sum(sum(1 - consts(win_size + 1 : end - win_size, win_size + 1 : end - win_size))) * (neb_size ^ 2);
...
%     end  
      ...
      win_mu = mean(winI , 1)';
      win_var = winI' * winI / neb_size - win_mu * win_mu' + (epsilon/neb_size) .* eye(c);
